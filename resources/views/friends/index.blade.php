                            ...
                            <div class="space-y-3">
                                @forelse($pendingFriendsFrom as $pendingFriendFrom)
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="space-x-2">
                                            <form action="{{route('friends.patch',$pendingFriendFrom)}}" method="post"
                                                  class="inline">
                                                @csrf
                                                @method('PATCH')
                                                <button class="text-indigo-600">Accept</button>
                                            </form>
                                            <form action="{{route('friends.destroy',$pendingFriendFrom)}}" method="post"
                                                  class="inline">
                                                @csrf
                                                @method('DELETE')
                            ...
                            <div class="space-y-3">
                                @forelse($pendingFriendsTo as $pendingFriendTo)
                                    <div class="flex items-center justify-between">
                                        ...
                                        <div class="space-x-2">
                                            <form action="{{route('friends.destroy',$pendingFriendTo)}}" method="post"
                                                  class="inline">
                                                @csrf
                                                @method('DELETE')
